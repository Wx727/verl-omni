        ...
        with marked_timer("update_weights", self.timing_raw, color="red"):
            ...
            if debug_enabled:
                logger.warning(
                    ...
                    "updates_since_last_sync=%d should_sync=%s standalone_paused=%s",
                    self.global_steps,
                    self.parameter_sync_step,
                    self._parameter_sync_debug_actor_updates_since_sync,
                    should_sync,
                    self._standalone_paused,
                )
            if should_sync:
                logger.warning(
            ...
            if self.sync_compatible and self._standalone_paused:
                ...
                if debug_enabled:
                    logger.warning(
                        "PARAM_SYNC_DEBUG resume_decision global_step=%d resuming_without_sync=%s",
                        self.global_steps,
                        not should_sync,
                    )
                self._resume_standalone_generation()
